...
function showOnly(elementToShow, selectionArea, quizContainer, questionCountSelection) {
    // console.log(`showOnly: Showing ${elementToShow?.id}, Hiding others`);
    try {
...
function showSubthemeSelectionScreen(theme, elements) {
    ...
    if (elements.subthemeSelectionContainer) {
        ...
        console.log("Visibilidade subtemas:", window.getComputedStyle(elements.subthemeSelectionContainer).display);
...
function showQuestion(questionData, elements) {
    console.log(`showQ: Idx ${currentQuestionIndex}`); isAnswered=false; selectedOptionElement=null; if(elements.confirmBtn)elements.confirmBtn.classList.remove('confirm-active'); if(elements.answerOptionsElement)elements.answerOptionsElement.querySelectorAll('.option-frame').forEach(f=>f.style.outline='none'); if(!questionData?.question){console.error("Questão inválida:",currentQuestionIndex,questionData);if(elements.messageArea)showMessage(elements.messageArea,"Erro questão",5000);showResults(elements);return;} if(elements.questionTextElement)elements.questionTextElement.innerText=questionData.question; else console.error("questionTextElement!"); if(elements.answerOptionsElement)elements.answerOptionsElement.innerHTML=''; else{console.error("answerOptionsElement!");return;} if(elements.messageArea)clearMessage(elements.messageArea); if(!questionData.options?.length){console.error("Opções inválidas:",questionData);if(elements.messageArea)showMessage(elements.messageArea,"Erro opções",5000);nextQuestion(elements);return;} let opts=[...questionData.options]; shuffleArray(opts); opts.forEach(opt=>{if(!opt?.text){console.warn("Opção inválida:",opt);return;} const oF=document.createElement('div');oF.className='option-frame';oF.dataset.optionText=opt.text; const fF=document.createElement('div');fF.className='front-face';fF.textContent=opt.text; const bF=document.createElement('div');bF.className='back-face'; const eS=document.createElement('span');eS.className='explanation';eS.textContent=opt.explanation||'Sem explicação.'; bF.appendChild(eS); oF.appendChild(fF);oF.appendChild(bF); oF.addEventListener('click',(e)=>selectAnswer(e,elements)); if(elements.answerOptionsElement)elements.answerOptionsElement.appendChild(oF);}); if(quizJustStarted){if(elements.quizBackBtn)elements.quizBackBtn.classList.remove('hide'); if(elements.quizMainMenuBtn)elements.quizMainMenuBtn.classList.remove('hide'); if(elements.confirmBtn){elements.confirmBtn.classList.remove('hide');elements.confirmBtn.disabled=true;} if(elements.nextBtn)elements.nextBtn.classList.add('hide'); if(elements.finishBtn)elements.finishBtn.classList.add('hide'); if(elements.endQuizEarlyBtn)elements.endQuizEarlyBtn.classList.add('hide');}else{if(elements.quizBackBtn)elements.quizBackBtn.classList.add('hide'); if(elements.quizMainMenuBtn)elements.quizMainMenuBtn.classList.add('hide'); if(elements.confirmBtn){elements.confirmBtn.classList.remove('hide');elements.confirmBtn.disabled=true;} if(elements.nextBtn)elements.nextBtn.classList.add('hide'); if(elements.finishBtn)elements.finishBtn.classList.add('hide'); if(elements.endQuizEarlyBtn)elements.endQuizEarlyBtn.classList.add('hide');} if(elements.questionContainer)elements.questionContainer.classList.remove('hide'); else console.error("questionContainer!"); updateProgressBar(elements);
}

function selectAnswer(event, elements) {
    if(isAnswered)return; if(elements.messageArea)clearMessage(elements.messageArea); const clickedFrame=event.currentTarget; if(!clickedFrame)return; if(elements.answerOptionsElement)elements.answerOptionsElement.querySelectorAll('.option-frame').forEach(f=>{if(f!==clickedFrame)f.classList.remove('selected');}); clickedFrame.classList.toggle('selected'); if(clickedFrame.classList.contains('selected')){selectedOptionElement=clickedFrame; if(elements.confirmBtn){elements.confirmBtn.disabled=false; elements.confirmBtn.classList.add('confirm-active');}}else{selectedOptionElement=null; if(elements.confirmBtn){elements.confirmBtn.disabled=true; elements.confirmBtn.classList.remove('confirm-active');}}
}
